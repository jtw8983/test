...
const List3 = () => {
    ...
    return (
        ...
        <div className="list-group mt-2 text-center">
            {
            arr?.map((v,i)=><button key={i} className={`list-group-item m-1 display-6 ${v.accept === 1 ? "list-group-item-success" : "list-group-item-warning"}`} onClick={()=>onClick(i)}>{v.title}</button>)
            } 
        </div>
        </>                  
    )
